...
const PeopleTab = () => {
    return <div className="grid mt-5  grid-cols-3 gap-4">
        {peoples.map((people) => {
    ...
    return (
        <div className='pb-32'>
            ...
            <div className="cursor-pointer sm:mx-[-20px] grid grid-cols-2 text-center py-3 font-semibold ">
                <div onClick={() => setSelectedTab('people')}
                    className={`${selectedTab === 'people' ? 'border-b-2 border-white text-white' : 'text-patchwork_black-50 '}  pb-3`} >People</div>
                <div onClick={() => setSelectedTab('activity')}
                    className={`${selectedTab === 'activity' ? 'border-b-2 border-white text-white' : 'text-patchwork_black-50 '}  pb-3`}>Activity</div>
            </div>
