...
const AboutYou = () => {
    return <>
        ...
        <div className="mb-3">
            <h3 className="font-semibold mb-1">What's your date of birth?</h3>
            <Input type='date'
                ...
                return <div key={index} className='mb-5' >
                    ...
                    <div className="flex flex-wrap items-center gap-2 gap-sm-4">
                        {interest.tags.map((tag, tagIndex) => {
                            return <InterestTag tag={tag} selectedTags={selectedTags} setSelectedTags={setSelectedTags} key={tagIndex} />
                        })}
                    </div>
                </div>
            })}
...
const ContributorsToFollow = () => {
    return <>
        <p className="my-6 text-patchwork_black-300">
            We've pre-selected the best contributors to follow based on your chosen interests.
        </p>
    ...
    return (
        <MainLayout className="px-3 sm:px-6 bg-patchwork_black-500 text-patchwork_white-900 min-h-screen">
            ...
            <div className="grid grid-cols-3 gap-2 gap-sm-5">
                {[1, 2, 3].map((_, index) => {
                    return <div className={`h-1 rounded-2xl w-full ${currentTab === index ? 'bg-patchwork_red-500' : 'bg-patchwork_black-400'}`} />
                })}
            </div>
            ...
            }} className="my-8 w-full" type="submit">
                Next
            </Button>
        </MainLayout>
    )
